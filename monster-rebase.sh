...
git add .
git commit -s -m ".github/workflows: ignore erroring out" -m "$CREDIT"
      ...
      - name: Upload ROM to PixelDrain
        ...
        run: |
          ...
          for ROM in out/UN1CA*.zip out/*target*.zip; do
            ...
            ./pd --api-key "$PIXELDRAIN_API_KEY" upload "$ROM" | sed 's|api/file|u|g'
          done
EOF
...
# 1. Directly replace the matrix line (preserves exact indentation)
perl -pi -e 's/target: \[o1s, p3s, r9s, t2s\]/build_type: [encrypted, decrypted]/' .github/workflows/build.yml
